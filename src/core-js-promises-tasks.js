  ...
  return new Promise((resolve, reject) => {
    ...
    promises.forEach((promise) => {
      promise
        ...
        .catch(() => {
          handledCount += 1;
          if (!resolved && handledCount === promises.length) {
...
function queuPromises(promises) {
  ...
  function chainPromises(promise, index) {
    return promise.then((value) => {
      result += value;
      if (index < promises.length - 1) {
        return chainPromises(promises[index + 1], index + 1);
      }
      return result;
    });
  }
  ...
  if (promises.length > 0) {
    return chainPromises(promises[0], 0);
  }
